...
public:
  ...
  virtual void visit_property(const char *label, ParameterBase &p,
    ...
    if (auto attr = getAttribute(label, "val")) {
      ...
      if (hasCommandStack(tags))
        p.setCommandStack(&m_project.getStack());
    } else
      m_report.addError(Severity::Warning, m_pathStack, m_currentComponentType,
                        "Error resolving Parameter \"" + std::string(label) +
                            "\"",
                        m_currentEntity);
  }
  virtual void visit_property(const char *label, std::string &str,
                              std::initializer_list<Tag> tags) override {
    if (!isPersistent(tags))
      return;
    if (auto attr = getAttribute(label, "val")) {
      ...
      m_report.addError(Severity::Warning, m_pathStack, m_currentComponentType,
                        "Error resolving string attribute \"" +
                            std::string(label) + "\"",
                        m_currentEntity);
  };
  ...
  virtual void visit_property(const char *label, float &val,
    ...
    if (auto attr = getAttribute(label, "val")) {
      ...
      m_report.addError(Severity::Warning, m_pathStack, m_currentComponentType,
                        "Error resolving float attribute \"" +
                            std::string(label) + "\"",
                        m_currentEntity);
  };
  ...
  virtual void visit_property(const char *label, EntityReference &ref,
    ...
    if (auto attr = getAttribute(label, "UUID")) {
      ...
      m_report.addError(Severity::Warning, m_pathStack, m_currentComponentType,
                        "Error resolving EntityReference attribute \"" +
                            std::string(label) + "\"",
                        m_currentEntity);
  }
  ...
  virtual bool beginList(const char *label) override {
    ...
    m_report.addError(Severity::Warning, m_pathStack, m_currentComponentType,
                      "Error resolving List \"" + std::string(label) + "\"",
                      m_currentEntity);
    return false;
  }
